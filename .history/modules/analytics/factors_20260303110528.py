    ...
    def _run_multistage(
        ...
        *,
        ...
        for idx, layer in enumerate(layer_specs):
            ...
            out[layer_name] = tmp[layer_name]
            out[f"{layer_name}_zscore"] = out[layer_name]
            available_nodes.append(layer_name)

        last_name = str(layer_specs[-1].get("name", "")).strip() or f"layer_{len(layer_specs)}"
        if last_name != "score":
            out["score"] = out[last_name]
        return out
