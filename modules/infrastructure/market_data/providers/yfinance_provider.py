...
class YFinancePriceProvider(BasePriceProvider):
    ...
    def __init__(self, batch_size: int = 100, yf_timeout: int = 60) -> None:
        self._batch_size = batch_size
        self._yf_timeout = yf_timeout
    ...
    def _download_candidate_matrix(
        ...
        *,
        ...
        for i in range(0, len(tickers), self._batch_size):
            ...
            try:
                ...
                self._logger.warning(
                    "Yahoo bulk download failed for batch %s...: %s", batch[:3], exc
                )
                continue

            close_prices = self._extract_close_frame(raw, tickers=batch)
        ...
        combined = pd.concat(batch_frames, axis=1)
